...
def main():
  ...
  parser.add_argument('--workload', type=str, default="object", help='Workload')
  parser.add_argument('--outcome', type=str, default="performance", help='Outcome metric') 
  parser.add_argument('--n_all', type=int, default=10, help='Sample budget (default: 10)')
  ...
  compo = ['bitrate', 'performance', 'powerConsumption']
  ...
  for j in range(n_runs):   
      ...
      top_perf_de = np.sort(Yr.powerConsumption.values)[-125]
      ...
      top_perf_sh = np.sort(Yr.powerConsumption.values)[-140]
      
      ## (2) Software first
      top_idx_sh = n_hs * (n_hard-1)
      top_perf_sh = np.sort(Yr.powerConsumption.values)[-130]
      ...
      ANN = MLPRegressor(random_state=0, activation='tanh', 
                         hidden_layer_sizes=32, learning_rate_init=1e-1, max_iter=500)

...
def main():
  ...
  parser.add_argument('--workload', type=str, default="soa", help='Workload')
  parser.add_argument('--outcome', type=str, default="performance", help='Outcome metric') 
  parser.add_argument('--n_all', type=int, default=5, help='Sample budget (default: 5)')
  ...
  Xr = X.copy()
  X.drop(['id', 'utilizedCoreFrequency', 'utilizedCores'], axis=1, inplace=True)

  Measure = pd.read_csv('fast/outcome/{}.csv'.format(workload), index_col=False)
  Measure.drop('id', axis=1, inplace=True)
  compo = ['cost', 'reliability', 'responseTime']
  Yr = Measure[compo].copy()

  cutoff = 0.5
  Yr.loc[Yr.eval('responseTime <= {}'.format(cutoff)), 'cons'] = 1
  Yr.loc[Yr.eval('responseTime > {}'.format(cutoff)), 'cons'] = 0
  Yr['comp'] = Yr['cons'] * Yr['reliability']
  Y = Yr['comp'].copy()
  ...
  kernel = RBF(length_scale=1, length_scale_bounds=(1e-5, 1e1)) 
  ...
  for j in range(n_runs):   
      ...
      top_perf_de = np.sort(Yr.reliability.values)[8]
      ...
      X_hs, Y_hs = X.values[:n_hs//100], Y.values[:n_hs//100]
      ...
      top_perf_hs = Yr.loc[top_idx_hs, 'reliability'].item()
      
      ## (2) Software first
      top_idx_sh = n_hs * (n_hard-1)
      top_perf_sh = np.sort(Yr.reliability.values)[10]
      ...
      top_perf_ei = Yr.loc[top_idx_ei, 'reliability'].item()  
      ...
      top_perf_rnd = Yr.loc[top_idx_rnd, 'reliability'].item()
      ...
      top_perf_co = Yr.loc[top_idx_co, 'reliability'].item()
      ...
      top_perf_hy = Yr.loc[top_idx_hy, 'reliability'].item()
      ...
      ANN = MLPRegressor(random_state=0, activation='tanh', 
                         hidden_layer_sizes=16, learning_rate_init=1e-2, max_iter=1000)
      ...
      top_perf_nn = Yr.loc[top_idx_nn, 'reliability'].item()
      ...
      top_perf_ga = Yr.loc[top_idx_ga, 'reliability'].item()
      
      ########################## 5. Optimal Value ###########################
      top_perf_opt = np.max(Yr.reliability.values)    

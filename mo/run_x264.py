...
def main():
  ...
  parser.add_argument('--workload', type=str, default="x264", help='Workload')
  parser.add_argument('--outcome', type=str, default="performance", help='Outcome metric') 
  parser.add_argument('--n_all', type=int, default=20, help='Sample budget (default: 20)')
  ...
  kernel = RBF(length_scale=1e-2, length_scale_bounds=(1e-3, 1e1)) 
  model=GaussianProcessRegressor(kernel=kernel, alpha=1e-1)
  ...
  for j in range(n_runs):   
      ...
      top_perf_de = np.sort(Yr.powerConsumption.values)[-150]
      ...
      top_perf_hs = np.sort(Yr.powerConsumption.values)[-160]
      
      ## (2) Software first
      top_idx_sh = n_hs * (n_hard-1)
      top_perf_sh = np.sort(Yr.powerConsumption.values)[-170]
      ...
      ## (1) Random sampling 
      seed_idx_rnd = np.argsort(Y)[-int(n_all*1.5):].tolist()
